...
1. **Pembelian → Penerimaan Pembelian** (`/admin/purchase-receipts`).
2. **Diharapkan:** kolom Status berupa label berwarna, tidak bisa diklik untuk diubah.
3. Bandingkan dengan Poin 9 langkah 3: status berubah otomatis dari proses QC.

### Poin 3 — Invoice pembelian tanpa OR
1. **PO-B:** buat PO dengan urutan form yang sama seperti Poin 8: Tanpa Referensi → Supplier A → Cabang Pusat → **Gudang Tujuan UAT Gudang Utama** (pilih yang berawalan **UAT**; ada juga "Gudang Utama" bawaan berkode GU001) → isi kartu item pertama yang sudah ada: `UAT-BRG-001`, **Qty 10** (Harga Satuan otomatis `10.000,00`, Pajak Eks sudah terpilih). Klik Buat PO, lalu **Setujui**. Buat QC (Purchase Order = PO-B; Gudang Penerimaan terkunci Utama; toggle **"Langsung Terbitkan Penerimaan Barang (GRN)"** biarkan **menyala**; di tabel item Diterima `10`, Lolos `10`) → **GRN-B** terbit, PO menjadi Completed.
2. **Invoice Pembelian → Buat** (`/admin/purchase-invoices/create`). Isi **dari atas ke bawah**:
   - **Bagian "Sumber Invoice":** **Supplier** `UAT Supplier A` → kolom **Order Request (OR) — Opsional: kosongkan** → **Cabang** terisi otomatis → di **Purchase Orders** (muncul setelah supplier dipilih) centang PO-B (berlabel **[Direct PO]**).
   - **Bagian data invoice:** **Invoice Number:** klik ikon Generate → **No. Invoice Supplier:** `SUP-INV-UAT-001` → **No. Faktur Pajak:** `010.000-26.00000001` → **Invoice Date:** biarkan hari ini → **Due Date:** terisi otomatis 30 hari setelah Invoice Date.
   - **Bagian "Silahkan Pilih Purchase Receipt"** (**di bawah** data invoice, bukan di atas): centang **GRN-B**. Tabel **Item Invoice** dan ringkasan nilai terisi otomatis; jangan diubah.
   - Gulir ke bawah: bagian **Status Invoice** hanya menampilkan "Draft" (terkunci). Bagian **Pemilihan COA** tertutup dan sudah terisi bawaan; biarkan.
   - Klik **Buat**. **Diharapkan:** tersimpan sebagai **Draft**; total **Rp111.000** (100.000 + PPN 11.000).
3. **Nomor ganda:** buat **PO-C/GRN-C** (qty 5) dengan langkah yang sama. Di Invoice → Buat isi seperti langkah 2 (Supplier A → centang PO-C → Generate Invoice Number → **No. Invoice Supplier `SUP-INV-UAT-001`** (sama dengan invoice B) → No. Faktur Pajak baru `010.000-26.00000003`), lalu centang **GRN-C** di bagian Purchase Receipt. Klik Buat. **Diharapkan:** muncul notifikasi merah "Invoice pembelian belum dapat disimpan" **dan** pesan "…sudah pernah digunakan untuk supplier ini" di bawah kolom; invoice tidak tersimpan. Ulangi dengan **No. Faktur Pajak yang sama dengan invoice B** (`010.000-26.00000001`) dan **No. Invoice Supplier diganti** `SUP-INV-UAT-002`; **diharapkan** ditolak dengan pesan serupa di bawah kolom No. Faktur Pajak.
4. Setelah itu isi nomor yang benar (**No. Invoice Supplier** `SUP-INV-UAT-002`, **No. Faktur Pajak** `010.000-26.00000002`) → tersimpan sebagai **Draft (Invoice-C)**.
...
1. Buka **Invoice-B** (draft dari Poin 3). Klik **Lihat Journal Entries**. **Diharapkan:** tidak ada jurnal. Buka Akuntansi → **Utang Usaha**: tidak ada baris untuk invoice ini.
...
3. Buka Permintaan Pembayaran → Buat, pilih Supplier A: **invoice draft tidak boleh muncul**.
4. Di Invoice-B klik **Posting Invoice** → **Ya, Posting Invoice**. **Diharapkan:** status **Terkirim**; tombol Ubah dan Hapus hilang; jurnal:
   - Dr *Penerimaan Barang Belum Tertagih* **100.000**
   - Dr *PPN Masukan* **11.000**
   - Cr *Hutang Dagang* **111.000**
   Coba buka `…/admin/purchase-invoices/{id}/edit` → **403 Forbidden**.
5. **Pembatalan:** posting **Invoice-C**, lalu klik **Batalkan Invoice** → isi Tanggal (hari ini) dan **Alasan** `Nomor faktur salah` → **Ya, Batalkan Invoice**. **Diharapkan:** status **Dibatalkan**; halaman menampilkan "Dibatalkan Pada / Oleh / Alasan"; ada jurnal balik (debit-kredit terbalik); Utang Usaha invoice itu hilang; **GRN-C bisa ditagihkan lagi** dengan nomor invoice supplier yang sama.
   Login `uat.keuangan`: tombol **Batalkan Invoice** **tidak boleh muncul**.
6. Coba **Batalkan Invoice** pada invoice yang **sudah dibayar** (dari Poin 5). **Diharapkan:** ditolak dengan pesan "sudah memiliki pembayaran vendor…".
7. **Terlambat:** buat invoice dengan **Invoice Date 60 hari lalu** dan **Due Date 30 hari lalu**. Ubah **Invoice Date lebih dulu, baru Due Date**, karena Due Date ikut terisi ulang otomatis setiap Invoice Date berubah. Lalu posting. Status berubah menjadi **Terlambat** setelah job harian (00:05) berjalan. Minta IT menjalankan `php artisan invoices:check-overdue`, atau cek keesokan harinya bila cron server aktif.
...
2. Bagian **Pilih Invoice yang akan Dibayar** (muncul setelah supplier dipilih): **Diharapkan:** invoice PO-D tampil dengan **"Sisa Hutang: Rp 71.928,00"**. **Centang** invoice itu; **Total Pembayaran** di atas otomatis menjadi Rp71.928 dan tidak bisa diubah. Kolom **Catatan Permintaan** kosongkan. Klik **Buat** → **Ajukan Persetujuan** → **Setujui** (Catatan Persetujuan opsional) → **Konfirmasi**. Ini **PR-1**.
3. Buat PR baru untuk supplier yang sama. **Diharapkan:** invoice tadi **tidak muncul lagi**.
4. Di halaman PR-1 klik **Buat Vendor Payment**. Bagian atas form (**Referensi Payment Request**, Nomor Pembayaran, Cabang Operasional, Payment Date, Vendor, dan **Pilih Invoice**) sudah terisi dari PR-1; jangan diubah. Lalu isi **dari atas ke bawah**:
   1. **Detail Pembayaran per Invoice → Jumlah Pembayaran** (tertulis "Jumlah Pembayaran Source"): ubah menjadi `50000`.
   ...
   4. **NTPN:** kosongkan. **COA** (kolom terakhir, **di bawah** bagian bank): terisi otomatis akun bank begitu Payment Method dipilih; pastikan yang terpilih akun bank (mis. `Bank UAT` dari A11) atau ganti bila perlu. Bagian **Pajak Impor** jangan diubah.
   5. Klik **Buat**. **Diharapkan:** VP status Partial; invoice **Dibayar Sebagian**; PR-1 **Dibayar Sebagian**; sisa hutang Rp21.928.
5. Buat Vendor Payment kedua dari PR-1 untuk sisa **Rp21.928**. **Diharapkan:** invoice **Lunas**; PR-1 **otomatis tertutup (Lunas)**.

> **Catatan interpretasi:** temuan asli menulis "permintaan ke-2 harusnya Rp21.928". Sistem sekarang **memblokir** PR ke-2 selama PR-1 masih menutup seluruh sisa (langkah 3). Tanyakan pelapor mana yang dimaksud sebelum menilai lulus/gagal.
