...
import { ChevronDown, Trash2, Tag, Percent, DollarSign, Package, AlertCircle, Plus } from 'lucide-react';
import { SearchableSelect, SelectOption } from '../OrderRequestForm/SearchableSelect';
import { SaleOrderItemRow, SaleOrderDependencies, ProductOption } from './types';
...
interface Props {
  ...
  onChangeItems: (items: SaleOrderItemRow[]) => void;
  isReferQuotation: boolean;
  onAddItem?: () => void;
  errors?: Record<string, string[]>;
}

export const SaleOrderItemTable: React.FC<Props> = ({
  items,
  currencySymbol,
  dependencies,
  onChangeItems,
  ...
  // Alphabetically sorted products with free stock indicator
  const productOptions: SelectOption[] = useMemo(() => {
    return (dependencies.products || [])
      .slice()
      .sort((a, b) => a.name.localeCompare(b.name))
      .map((p) => {
        const stockLabel = p.free_stock > 0 ? `Stok: ${p.free_stock} ${p.uom?.abbreviation || 'PCS'}` : 'Stok Kosong';
        const priceLabel = `Std: ${formatCurrency(p.sell_price, currencySymbol)}`;
        return {
          value: p.id,
          label: p.name,
          badge: p.sku,
          sublabel: `${stockLabel} | ${priceLabel}`,
        };
      });
  }, [dependencies.products, currencySymbol]);

  // Handle item updates
  const handleItemChange = (index: number, updatedFields: Partial<SaleOrderItemRow>) => {
    const newItems = [...items];
    const current = newItems[index];

    // If product changed, auto-fill unit, sell_price, free_stock, etc.
    if (updatedFields.product_id !== undefined && updatedFields.product_id !== current.product_id) {
      ...
      if (prod) {
        ...
        updatedFields.free_stock = prod.free_stock || 0;
  ...
  const handleDeleteItem = (index: number) => {
    if (items.length <= 1) {
      alert('Minimal harus ada 1 item dalam pesanan penjualan.');
      return;
    }
              ...
              <div className="p-4 border-t border-gray-100 bg-white space-y-4 animate-in fade-in-50 duration-150">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Contoh: Pengiriman batch, spesifikasi tambahan..."
                  />
                </div>
              </div>
            )}
      ...
      {!isReferQuotation && onAddItem && (
        <div className="pt-2">
          ...
          >
            ...
            <span>Tambah Item Pesanan Baru</span>
          </button>
        </div>
      )}
    </div>
