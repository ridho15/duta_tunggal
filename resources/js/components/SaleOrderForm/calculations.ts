import { CalculationResult, SaleOrderItemRow, SaleOrderSummary } from './types';

/**
 * Calculates financial breakdown for an individual sales order item.
 */
export function calculateItemPreview(
  ...
  let taxRate = 0;
  if (
    normalizedTaxType === 'inklusif' ||
    normalizedTaxType === 'eksklusif' ||
    normalizedTaxType === 'eklusif' ||
    normalizedTaxType === 'ppn included' ||
    normalizedTaxType === 'ppn excluded'
  ) {
  ...
  if (
    normalizedTaxType === 'inklusif' ||
    normalizedTaxType === 'ppn included' ||
    normalizedTaxType === 'none' ||
    normalizedTaxType === 'non pajak'
  ) {
...
/**
 * Calculates document-wide sales order summary.
 */
export function calculateSaleOrderSummary(
