<style>
    ...
    .dt-po-control:focus,
    .dt-po-inline-input:focus,
    .dt-po-inline-select:focus {
    ...
    .dt-po-button:hover:not(:disabled) {
        background: #f8fafc
    }

    .dt-po-button.primary {
        border-color: #2563eb;
        background: #2563eb;
        color: #fff
    }

    .dt-po-button.primary:hover:not(:disabled) {
        background: #1d4ed8
    }

    .dt-po-button:disabled,
    .dt-po-control:disabled {
        cursor: not-allowed;
        opacity: .55;
        background: #f8fafc
    }
    ...
    .dt-po-loading-spinner {
        width: 14px;
        height: 14px;
        border: 2px solid #bfdbfe;
        border-top-color: #2563eb;
        border-radius: 999px;
        animation: dt-po-spin .7s linear infinite
    }
    ...
    .dt-po-table-wrap.is-loading .dt-po-table {
        opacity: .55
    }

    .dt-po-loading-overlay {
        position: absolute;
        inset: 0;
        z-index: 4;
        display: flex;
    ...
    .dt-po-table th.dt-po-action-col {
        background: #f8fafc;
        z-index: 1
    }
    ...
    .dt-po-icon-button:hover {
        background: #eff6ff;
        border-color: #93c5fd
    }

    .dt-po-icon-button.danger {
        color: #dc2626
    }

    .dt-po-icon-button.danger:hover {
        background: #fef2f2;
        border-color: #fecaca
    }
    ...
    .dt-po-apply-button:hover:not(:disabled) {
        background: #dbeafe
    }

    .dt-po-apply-button:disabled {
        cursor: not-allowed;
        opacity: .55
    }
    ...
    .dt-po-inline-editor .select2-container {
        width: 100% !important;
        min-width: 0
    }

    .dt-po-inline-editor .select2-container .select2-selection--single {
        height: 38px;
        border: 1px solid #d1d5db;
        border-radius: 10px;
        background: #fff
    ...
    .dt-po-inline-editor .select2-container .select2-selection--single .select2-selection__rendered {
        line-height: 36px;
        padding-left: 11px;
        padding-right: 34px;
        font-size: 13px;
    ...
    .dt-po-inline-editor .select2-container .select2-selection--single .select2-selection__arrow {
        height: 36px;
        right: 8px
    }

    .dt-po-inline-editor .select2-container--focus .select2-selection--single {
        border-color: #2563eb;
    ...
    .dt-po-inline-field.wide {
        grid-column: span 6
    }

    .dt-po-inline-field label {
        font-size: 12px;
        font-weight: 800;
        color: #475569
    }
    ...
    .dt-po-tax-select:disabled {
        background-image: none
    }

    .dt-po-inline-input[readonly],
    .dt-po-inline-select:disabled {
        background: #f8fafc;
        color: #64748b;
        cursor: not-allowed
    }
    ...
    .dt-po-money-wrap .dt-po-inline-input {
    ...
    .dt-po-footer-total span:last-child {
        background: #f8fafc;
        border: 1px solid #e5e7eb;
        border-radius: 10px;
        padding: 8px 12px
    ...
    .dt-po-large-repeater,
    .dt-po-large-repeater .fi-fo-repeater-item {
        height: 0 !important;
    ...
    .dt-po-large-repeater [data-dt-or-bulk-toggle],
    .dt-po-large-repeater [data-dt-or-add-item] {
        display: none
    }
...
<div class="dt-po-panel" data-dt-po-navigator data-current-search="{{ e($search) }}"
    data-current-tax="{{ e((string) ($taxFilter ?? '')) }}" data-current-source="{{ e((string) ($sourceFilter ?? '')) }}"
    data-current-cabang="{{ e((string) ($cabangFilter ?? '')) }}" x-data="{
        ...
        init() {
            this.rootElement = this.$root || this.rootElement || document.querySelector('[data-dt-po-navigator]');
            const root = this.rootEl();
            if (!root) return;

            this.searchValue = root.dataset.currentSearch || '';
            this.taxValue = root.dataset.currentTax || '';
            this.sourceValue = root.dataset.currentSource || '';
        ...
        rootEl() {
            return this.$root || this.rootElement || document.querySelector('[data-dt-po-navigator]');
        },
        ...
        async initInlineSelects(key) {
            const root = this.rootEl();
            const editor = root?.querySelector('[data-dt-po-inline-editor=' + CSS.escape(String(key)) + ']');
            if (!editor) return;
    ...
    <div class="dt-po-toolbar" style="margin-top:12px">
        <div class="dt-po-toolbar-left">
            <div class="dt-po-search-wrap">
                ...
                <input type="search" class="dt-po-control dt-po-search" placeholder="Search item / product / source..."
                    x-model="searchValue"
    ...
    <div class="dt-po-filter-drawer" x-show="filterOpen" x-cloak>
        <div class="dt-po-field">
            ...
            <select class="dt-po-control dt-po-filter-select" x-model="taxValue"
                x-on:change="setNavigatorState('_purchase_order_item_tax_filter', taxValue)">
        ...
        <div class="dt-po-field">
            ...
            <select class="dt-po-control dt-po-filter-select" x-model="sourceValue"
                x-on:change="setNavigatorState('_purchase_order_item_source_filter', sourceValue)">
        ...
        <div class="dt-po-field">
            ...
            <select class="dt-po-control dt-po-filter-select" x-model="cabangValue"
                x-on:change="setNavigatorState('_purchase_order_item_cabang_filter', cabangValue)">
    ...
    <div class="dt-po-chips">
        ...
        @endif
        @if ($sourceFilter)
            <span class="dt-po-chip">Sumber:
                {{ $sourceFilter === 'order_request' ? 'Dari Order Request' : 'Manual' }}</span>
        @endif
    ...
    <div class="dt-po-count-row">
        <div>Showing {{ number_format($matchedCount, 0, ',', '.') }} of {{ number_format($totalItems, 0, ',', '.') }}
            items</div>
        <template x-if="isLoading">
            <div class="dt-po-loading-status"><span class="dt-po-loading-spinner"></span><span
                    x-text="loadingMessage"></span></div>
        </template>
    </div>
    ...
    <div class="dt-po-table-wrap" x-bind:class="{ 'is-loading': isLoading }">
        <template x-if="isLoading">
            <div class="dt-po-loading-overlay">
                <div class="dt-po-loading-card"><span class="dt-po-loading-spinner"></span><span
                        x-text="loadingMessage"></span></div>
            </div>
        </template>
            ...
            <tbody>
                @forelse ($rows as $row)
                    <tr class="dt-po-row" data-dt-po-row="{{ $row['key'] }}"
                        ...
                        <td>{{ $row['number'] }}</td>
                        <td class="dt-po-product">
                            {{ $row['product'] }}<small>{{ $row['refer_item_model_id'] ? 'OR Item #' . $row['refer_item_model_id'] : 'Manual item' }}</small>
                        </td>
                        <td>
                            <span
                                class="dt-po-badge {{ $row['is_order_request_backed'] ? '' : 'manual' }}">{{ $row['source'] }}</span>
                        ...
                        <td class="dt-po-action-col">
                            <button type="button" class="dt-po-icon-button danger"
                                x-on:click="removeItem(@js($row['key']))" @disabled($row['is_order_request_backed'])
                                title="{{ $row['is_order_request_backed'] ? 'Item dari Order Request tidak bisa dihapus' : 'Hapus item' }}">x</button>
                        </td>
                    </tr>
                    <tr class="dt-po-detail-row" wire:key="dt-po-detail-{{ $row['key'] }}"
                        x-show="expandedKey === @js($row['key'])" x-cloak>
                        <td colspan="10">
                                ...
                                <div class="dt-po-detail-summary">
                                    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                                        ...
                                        @if ($row['is_dirty'])
                                            ...
                                            <span class="dt-po-dirty-helper">Klik Simpan Perubahan Item agar masuk ke
                                                total/summary.</span>
                                        @else
                                            <span class="dt-po-idle-helper">Tidak ada perubahan item. Ubah
                                                qty/harga/diskon/pajak jika ingin menyimpan perubahan item.</span>
                                        @endif
                                        @if (!empty($row['validation_errors']))
                                            <span class="dt-po-error-badge">Item belum valid</span>
                                        @endif
                                    </div>
                                    <button type="button" class="dt-po-apply-button"
                                ...
                                <div class="dt-po-inline-editor" data-dt-po-inline-editor="{{ $row['key'] }}">
                                    ...
                                    <div class="dt-po-inline-grid">
                                        <div class="dt-po-inline-field wide">
                                            <label>Product</label>
                                            <select class="dt-po-inline-select" data-dt-po-inline-select
                                                data-field="product_id" data-search-method="products"
                                                ...
                                                @foreach ($row['product_options'] as $id => $label)
                                                    <option value="{{ $id }}" @selected((string) $row['product_id'] === (string) $id)>
                                                        {{ $label }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            ...
                                            <input type="number" step="0.01" min="0"
                                                class="dt-po-inline-input" value="{{ $row['quantity'] }}"
                                                x-bind:disabled="isLoading"
                                                x-on:input.debounce.500ms="updateInlineItem(@js($row['key']), 'quantity', $event.target.value)">
                                        </div>
                                        <div class="dt-po-inline-field">
                                            <label>Unit</label>
                                            <input type="text" class="dt-po-inline-input"
                                                value="{{ $row['unit'] }}" readonly>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            <label>Mata Uang</label>
                                            <select class="dt-po-inline-select" data-dt-po-inline-select
                                                data-field="currency_id" data-search-method="currencies"
                                                ...
                                                @foreach ($row['currency_options'] as $id => $label)
                                                    <option value="{{ $id }}" @selected((string) $row['currency_id'] === (string) $id)>
                                                        {{ $label }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            ...
                                            <div class="dt-po-money-wrap">
                                                ...
                                                <input type="text" class="dt-po-inline-input"
                                                    value="{{ $row['unit_price'] }}" data-dt-po-inline-money
                                                    ...
                                                    @readonly($row['is_order_request_backed'])>
                                            </div>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            <label>Discount (%)</label>
                                            <input type="number" step="0.01" min="0" max="100"
                                                class="dt-po-inline-input" value="{{ $row['discount'] }}"
                                                x-bind:disabled="isLoading || @js($row['is_order_request_backed'])"
                                        ...
                                        <div class="dt-po-inline-field">
                                            <label>Tipe Pajak</label>
                                            <select class="dt-po-inline-select dt-po-tax-select"
                                                x-on:change="updateInlineItem(@js($row['key']), 'tipe_pajak', $event.target.value)"
                                                @disabled($row['is_order_request_backed'])>
                                                @foreach ($taxOptions as $value => $label)
                                                    <option value="{{ $value }}" @selected($row['tipe_pajak'] === $value)>
                                                        {{ $label }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            <label>Tax (%)</label>
                                            <input type="number" class="dt-po-inline-input"
                                                value="{{ $row['tax'] }}" readonly>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            <label>Discount nominal</label>
                                            <div class="dt-po-money-wrap"><span
                                                    class="dt-po-money-prefix">{{ $row['currency_symbol'] }}</span><input
                                                    type="text" class="dt-po-inline-input"
                                                    value="{{ $row['discount_nominal'] }}" data-dt-po-inline-money
                                                    readonly></div>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            ...
                                            <div class="dt-po-money-wrap"><span
                                                    class="dt-po-money-prefix">{{ $row['currency_symbol'] }}</span><input
                                                    type="text" class="dt-po-inline-input"
                                                    value="{{ $row['tax_nominal'] }}" data-dt-po-inline-money
                                                    readonly></div>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            <label>Total</label>
                                            <div class="dt-po-money-wrap"><span
                                                    class="dt-po-money-prefix">{{ $row['currency_symbol'] }}</span><input
                                                    type="text" class="dt-po-inline-input"
                                                    value="{{ $row['total'] }}" data-dt-po-inline-money readonly>
                                            </div>
                                        </div>
                                        <div class="dt-po-inline-field">
                                            ...
                                            <div class="dt-po-money-wrap"><span
                                                    class="dt-po-money-prefix">{{ $row['currency_symbol'] }}</span><input
                                                    type="text" class="dt-po-inline-input"
                                                    value="{{ $row['subtotal'] }}" data-dt-po-inline-money readonly>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                @empty
                    <tr wire:key="dt-po-empty-row">
                        <td class="dt-po-empty" colspan="10">Belum ada item. Klik Tambah Item untuk mulai mengisi
                            Purchase Order.</td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="dt-po-footer">
        ...
        <div class="dt-po-footer-total">Subtotal Preview: <span>Rp
                {{ number_format($totalSubtotal, 2, ',', '.') }}</span></div>
    </div>
</div>
