@php
    ...
    $rows = $record
        ? $record->permissions
            ->sortBy('name')
            ->values()
            ->map(function ($permission) use ($permissionDescriptions, $permissionModules) {
    ...
    x-data="{
        ...
        get filteredRows() {
            ...
            return this.items.filter((row) => {
                return [row.name, row.module, row.description]
                    .join(' ')
                    .toLowerCase()
...
>
    <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
        <p class="text-sm font-semibold text-gray-800">Ringkasan Hak Akses</p>
        @if(count($rows) === 0)
            <p class="mt-1 text-sm text-gray-600">Role ini belum memiliki permission aktif.</p>
        @else
            <p class="mt-1 text-sm text-gray-600">
                Role ini memiliki {{ count($rows) }} permission aktif dan dapat digunakan oleh {{ $record->users->count() }} user.
            </p>
        @endif
    </div>
        ...
        <div class="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                <div class="w-full lg:max-w-md">
                    <label class="mb-1 block text-sm font-medium text-gray-700" for="role-permission-search">Cari permission</label>
                    <input
