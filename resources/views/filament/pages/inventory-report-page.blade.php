...
@media print {
    ...
    .fi-topbar, .fi-sidebar, .fi-page-header,
    .premium-filter-card, .premium-tab-bar, .premium-table-card, .premium-btn-print { display: none !important; }
    body { margin: 0; padding: 20px; }
    .inv-print-header { display: block !important; text-align: center; margin-bottom: 20px; }
}
...
<div class="inv-print-header">
    ...
    <p>
        @if($show_movement_history) History Movement Stok
        @elseif($show_aging_stock) Aging Stock Analysis
        @else Stok Barang per Gudang
        @endif
...
<div class="space-y-5 no-print-wrapper" style="margin-top: 10px;">
    ...
    <div class="no-print premium-header">
        ...
        <div class="flex items-start justify-between gap-4 relative z-10" style="display: flex; justify-content: space-between; align-items: flex-start; gap: 16px;">
            <div>
                <div class="flex items-center gap-3 mb-1" style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
            ...
            <div class="flex flex-col gap-2 flex-shrink-0" style="display: flex; flex-direction: column; gap: 8px; flex-shrink: 0;">
    ...
    <div class="premium-tab-bar no-print">
        {{-- Stok per Gudang --}}
        <button
            wire:click="$set('show_movement_history', false); $set('show_aging_stock', false)"
        ...
        {{-- History Movement --}}
        <button
            wire:click="$set('show_movement_history', true); $set('show_aging_stock', false)"
            class="premium-tab-btn {{ $show_movement_history ? 'active' : '' }}"
            id="tab-movement">
        ...
        {{-- Aging Stock --}}
        <button
            wire:click="$set('show_aging_stock', true); $set('show_movement_history', false)"
            class="premium-tab-btn {{ $show_aging_stock ? 'active' : '' }}"
            id="tab-aging">
        ...
        <div class="p-6" style="padding: 24px;">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div wire:key="filter-gudang" wire:ignore class="space-y-1.5" style="margin-bottom: 12px;">
                    ...
                    <select wire:model.live="warehouse_id" id="select-gudang" data-inv-select2 data-placeholder="— Semua Gudang —"
                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:border-teal-500 focus:ring-1 focus:ring-teal-500 transition">
                ...
                <div wire:key="filter-produk" wire:ignore class="space-y-1.5" style="margin-bottom: 12px;">
                    ...
                    <select wire:model.live="product_id" id="select-produk" data-inv-select2 data-placeholder="— Semua Produk —"
                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:border-teal-500 focus:ring-1 focus:ring-teal-500 transition">
            ...
            @if($warehouse_id || $product_id || ($show_movement_history && $start_date))
                <div class="mt-4 flex flex-wrap gap-2" style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 16px;">
                    @if($warehouse_id)
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-teal-50 text-teal-700 border border-teal-200 dark:bg-teal-900/30 dark:text-teal-300 dark:border-teal-700" style="padding: 4px 10px; border-radius: 9999px;">
    ...
    @php
        if ($show_movement_history) {
            ...
            $stats = [
                ['label' => 'Total Movement', 'value' => number_format($totalMovement), 'sub' => 'entri movement', 'icon' => 'arrow-path', 'color' => 'teal',    'badge' => null],
                ['label' => 'Total Masuk',    'value' => number_format($totalIn),       'sub' => 'transaksi IN',   'icon' => 'arrow-down-tray', 'color' => 'emerald', 'badge' => 'in'],
                ['label' => 'Total Keluar',   'value' => number_format($totalOut),      'sub' => 'transaksi OUT',  'icon' => 'arrow-up-tray',   'color' => 'rose',    'badge' => 'out'],
            ];
            ...
            foreach ((clone $baseStockQ)->with('product')->get() as $stock) {
                ...
                if (!$lastMov) { $deadItems++; continue; }
                $days = now()->diffInDays(\Carbon\Carbon::parse($lastMov));
                if ($days > 180) $deadItems++;
                elseif ($days > 90) $slowItems++;
            }
            $stats = [
                ['label' => 'Total Item',   'value' => number_format($totalItems), 'sub' => 'item stok', 'icon' => 'archive-box', 'color' => 'teal',   'badge' => null],
                ['label' => 'Slow Moving',  'value' => number_format($slowItems),  'sub' => '> 90 hari', 'icon' => 'clock',       'color' => 'amber',  'badge' => 'warning'],
                ['label' => 'Dead Stock',   'value' => number_format($deadItems),  'sub' => '> 180 hari', 'icon' => 'exclamation-triangle', 'color' => 'rose', 'badge' => 'danger'],
            ];
        } else {
            $baseStockQ = \App\Models\InventoryStock::query()
                ->when($warehouse_id, fn($q) => $q->where('warehouse_id', $warehouse_id))
            ...
            $stats = [
                ['label' => 'Total Item Stok', 'value' => number_format($totalItems),  'sub' => 'item terdaftar', 'icon' => 'archive-box',   'color' => 'teal',   'badge' => null],
                ['label' => 'Stok Normal',     'value' => number_format($normalItems), 'sub' => 'di atas minimum', 'icon' => 'check-circle', 'color' => 'emerald','badge' => 'normal'],
                ['label' => 'Stok Rendah',     'value' => number_format($lowItems),    'sub' => 'perlu restock',  'icon' => 'exclamation-triangle', 'color' => 'rose', 'badge' => 'danger'],
            ];
        }

        $colorMap = [
            'teal'    => ['bg' => 'bg-teal-100 dark:bg-teal-900/40',    'text' => 'text-teal-600 dark:text-teal-400',    'val' => 'text-teal-700 dark:text-teal-300'],
            'emerald' => ['bg' => 'bg-emerald-100 dark:bg-emerald-900/40','text'=> 'text-emerald-600 dark:text-emerald-400','val'=> 'text-emerald-700 dark:text-emerald-300'],
            'rose'    => ['bg' => 'bg-rose-100 dark:bg-rose-900/40',    'text' => 'text-rose-600 dark:text-rose-400',    'val' => 'text-rose-700 dark:text-rose-300'],
            'amber'   => ['bg' => 'bg-amber-100 dark:bg-amber-900/40',  'text' => 'text-amber-600 dark:text-amber-400',  'val' => 'text-amber-700 dark:text-amber-300'],
        ];
    @endphp
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 no-print" style="margin-bottom: 24px;">
        @foreach($stats as $stat)
            ...
            <div class="premium-stat-card">
                <div class="flex items-center justify-center w-12 h-12 rounded-xl {{ $c['bg'] }} {{ $c['text'] }} flex-shrink-0" style="width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 12px;">
                    @switch($stat['icon'])
                        @case('archive-box') <x-heroicon-o-archive-box class="w-6 h-6" /> @break
                        @case('arrow-path') <x-heroicon-o-arrow-path class="w-6 h-6" /> @break
                        @case('arrow-down-tray') <x-heroicon-o-arrow-down-tray class="w-6 h-6" /> @break
                        @case('arrow-up-tray') <x-heroicon-o-arrow-up-tray class="w-6 h-6" /> @break
                        @case('clock') <x-heroicon-o-clock class="w-6 h-6" /> @break
                        @case('check-circle') <x-heroicon-o-check-circle class="w-6 h-6" /> @break
                        @case('exclamation-triangle') <x-heroicon-o-exclamation-triangle class="w-6 h-6" /> @break
                        @default <x-heroicon-o-chart-bar class="w-6 h-6" />
                    @endswitch
                </div>
                <div class="min-w-0">
    ...
    <div class="premium-table-card">
        ...
        <div class="premium-card-header" style="justify-content: space-between; display: flex; align-items: center; width: 100%;">
            <div class="flex items-center gap-3" style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    ...
                    @elseif($show_aging_stock)
                        ...
                        <p class="premium-card-subtitle">Analisis umur stok untuk deteksi slow-moving & dead stock</p>
                    @else
            ...
            <div class="flex items-center gap-2 flex-shrink-0" style="display: flex; align-items: center; gap: 8px;">
                {{-- Active mode badge --}}
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border"
                    ...
                    @else                       background-color: #ecfdf5; color: #065f46; border-color: #a7f3d0;
                    @endif">
                    @if($show_movement_history) Movement
                    @elseif($show_aging_stock)  Aging
                    @else Stok
                    @endif
                </span>
