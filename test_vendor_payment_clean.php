...
echo '=== TEST PEMBAYARAN VENDOR PAYMENT - INVOICE BERSIH ===' . PHP_EOL . PHP_EOL;

// Gunakan invoice yang baru dibuat (ID 4)
$invoice = \App\Models\Invoice::find(4);

if (!$invoice) {
    echo '❌ Invoice test tidak ditemukan' . PHP_EOL;
    exit;
}
...
if (!$accountPayable) {
    echo '❌ Account Payable tidak ditemukan' . PHP_EOL;
    exit;
}
...
// Test 1: Pembayaran Partial (60% dari total)
$partialAmount = round($invoice->total * 0.6);
echo '=== TEST 1: PEMBAYARAN PARTIAL (60%) ===' . PHP_EOL;
...
$partialPayment = \App\Models\VendorPayment::create([
    ...
    'notes' => 'Test pembayaran partial 60%'
]);
...
echo '=== TEST 2: PEMBAYARAN LUNAS (SISA ' . number_format($remainingAmount, 0, ',', '.') . ') ===' . PHP_EOL;
...
echo 'ANALISIS AKUNTANSI PEMBAYARAN:' . PHP_EOL;
echo '✅ Debit Hutang Dagang = Mengurangi liability (utang ke supplier)' . PHP_EOL;
echo '✅ Credit Bank/Cash = Mengurangi kas/bank' . PHP_EOL;
echo '✅ Prinsip: Assets ↓, Liabilities ↓ (seimbang)' . PHP_EOL . PHP_EOL;
