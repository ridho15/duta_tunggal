...
test('role detail page shows permission capabilities and descriptions', async ({ page }) => {
  await navigate(page, '/admin/roles/2');

  await expect(page.getByText('Informasi Role')).toBeVisible({ timeout: 10_000 });
  await expect(page.getByRole('heading', { name: 'Hak Akses Role' })).toBeVisible();
  await expect(page.getByText('Ringkasan Hak Akses')).toBeVisible();
  ...
  const permissionArea = page.locator('#role-permission-search').locator('xpath=ancestor::div[contains(@class, "space-y-3")]');
