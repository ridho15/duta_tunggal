...
const APP_URL = 'http://127.0.0.1:8009';
...
test.describe('Quotation Form UI refresh', () => {
  test('create page renders requested quotation item UX and locked calculated fields', async ({ page }) => {
    const pageErrors = [];
    ...
    await expect(page.getByRole('radio', { name: /Inklusif/i })).toBeVisible();
    await expect(page.getByRole('radio', { name: /Eklusif/i })).toBeVisible();

    // Validate repeater is expandable/collapsible.
    await expect(page.getByRole('button', { name: /Sembunyikan/i }).first()).toBeVisible();

    await expect(page.getByRole('textbox', { name: /^Unit Price\*/i }).first()).toBeVisible();
    await expect(page.getByRole('textbox', { name: /^Total \(Harga x Qty\)\*/i }).first()).toBeVisible();
    await expect(page.getByRole('spinbutton', { name: /^Discount %$/i }).first()).toBeVisible();
    await expect(page.getByRole('textbox', { name: /^Discount \(Nominal\)/i }).first()).toBeVisible();
    await expect(page.getByRole('textbox', { name: /^Nominal Pajak$/i }).first()).toBeVisible();
    await expect(page.getByRole('textbox', { name: /^Subtotal$/i }).first()).toBeVisible();
    ...
    await page.screenshot({
      path: 'tests/playwright/screenshots/quotation-form-ui-refresh.png',
      fullPage: true,
    });
    ...
    // Note: textareaFormComponent errors are pre-existing and not related to our changes
    const runtimeProblems = [...pageErrors, ...consoleWarnings].filter((line) =>
      /textareaFormComponent|state is not defined/i.test(line),
    );

    // Only fail on NEW errors, not pre-existing textarea component issues
    const newErrors = runtimeProblems.filter(line =>
      !/textareaFormComponent|state is not defined/i.test(line)
    );
