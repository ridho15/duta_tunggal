...
class SalesFlowFilamentTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected ChartOfAccount $goodsDeliveryCoa;
    ...
    protected function setUp(): void
    {
        ...
        if (! $goodsDeliveryCoa) {
            $goodsDeliveryCoa = ChartOfAccount::factory()->create([
                'code' => '1140.20',
        ...
        $this->goodsDeliveryCoa = $goodsDeliveryCoa;
    ...
    /** @test */
    public function complete_sales_flow_via_filament_forms()
    {
        // ==========================================
        // STEP 1: CREATE SALES ORDER (Filament Form Simulation)
        // ==========================================
        ...
        // ==========================================
        // STEP 2: CREATE DELIVERY ORDER (Filament Form Simulation)
        // ==========================================
        ...
        // ==========================================
        // STEP 3: POST DELIVERY ORDER (Reduce Inventory)
        // ==========================================
    ...
    InventoryStock::whereKey($inventoryStock->id)->decrement('qty_available', 10);
        ...
        // Note: StockMovement may not be created automatically by DeliveryOrderService

        // ==========================================
        // STEP 4: CREATE INVOICE (Filament Form Simulation)
        // ==========================================
        ...
        $this->assertEquals([$deliveryOrder->id], $invoice->delivery_orders);
        ...
        // ==========================================
        // STEP 5: CREATE CUSTOMER RECEIPT (Payment)
        // ==========================================
    ...
    // 2. Delivery Order: Barang Terkirim Debit, Inventory Credit (2 entries)
    // 3. Invoice: AR Debit, Revenue Credit, COGS Debit, Barang Terkirim Credit (4 entries)
    // 4. Payment: Cash Debit, AR Credit (2 entries)
    $this->assertCount(10, $journalEntries);

        // Check delivery order entries (Barang Terkirim and Inventory)
        $goodsDeliveryDebit = $journalEntries
            ->where('coa_id', $this->goodsDeliveryCoa->id)
            ->where('debit', 100000)
            ->where('credit', 0)
            ->where('journal_type', 'sales')
            ->first();
        $this->assertNotNull($goodsDeliveryDebit);

        $inventoryCredit = $journalEntries
        ...
        // Invoice should also recognize COGS and release Barang Terkirim
        $cogsDebit = $journalEntries
            ->where('coa_id', $this->cogsCoa->id)
            ->where('debit', 100000)
            ->where('credit', 0)
            ->where('journal_type', 'sales')
            ->first();
        ...
        $goodsDeliveryCredit = $journalEntries
            ->where('coa_id', $this->goodsDeliveryCoa->id)
            ->where('debit', 0)
            ->where('credit', 100000)
            ->where('journal_type', 'sales')
            ->first();
        $this->assertNotNull($goodsDeliveryCredit);

        // Check payment entries (Cash and AR)
        $cashDebit = $journalEntries
            ->where('debit', 150000)
            ->where('credit', 0)
            ->where('journal_type', 'Sales')
            ->first();
    ...
    // Check Barang Terkirim balance (should net to zero after invoicing)
    $this->goodsDeliveryCoa->load('journalEntries');
    $this->assertEquals(0, $this->goodsDeliveryCoa->calculateEndingBalance());
    ...
    $this->assertEquals(700000, $totalDebit); // Opening 200k + DO 100k + Invoice (150k AR + 100k COGS) + Payment 150k
    $this->assertEquals(700000, $totalCredit); // Opening 200k + DO Inventory 100k + Invoice (150k Revenue + 100k Barang Terkirim) + Payment 150k
    }
}